...
(() => {
  ...
  let aiDraft='',aiBusy=false,aiError='',aiStatus='unknown',aiAbort=null,sampleFn=null;

  /* Published as an artifact, Claude is reached through the page's own
     sample capability rather than an HTTP backend — there is no server
     behind the site, the downloaded file or the artifact. */
  const SPARK_BRIEF = [
    'You are helping someone with ADHD find something they would actually want to do,',
    'or make a task they are avoiding more appealing. Interest is not a precondition for',
    'them, it is a product of starting under some stakes, so suggest concrete ways in',
    'rather than encouragement. Be specific and warm but not saccharine. Offer at most',
    'two or three concrete directions, each one sentence. Under 120 words unless asked',
    'for more. Never tell them to just focus or to try harder.',
  ].join(' ');
  async function ensureSample(){
    if(sampleFn!==null)return sampleFn;
    try{sampleFn=(typeof claude!=='undefined'&&claude&&claude.use)?await claude.use('sample'):null;}
    catch{sampleFn=null;}
    return sampleFn;
  }
  function aiView(){
    ...
    ${aiStatus==='unavailable'?'<p class="sp-notice">Claude is not reachable in this copy — the offline file and the public site have no connection to it. Open the artifact version if you want this part; everything else here works either way.</p>':aiStatus==='missing'?'<p class="sp-notice">Claude is not connected yet. The server needs an Anthropic API key. Built-in ideas still work.</p>':aiStatus==='unavailable'?'<p class="sp-notice">The Claude connection is unavailable on this version. Built-in ideas still work.</p>':''}
    ...
    <div class="sp-row">${button('ai-send',aiBusy?'Getting a reply…':'Send to Claude',`class="sp-primary" ${aiBusy||aiStatus==='unavailable'||aiStatus==='missing'?'disabled':''}`)}${aiBusy?button('ai-cancel','Cancel reply'):''}${button('projects','Browse built-in ideas')}${button('choose','Back')}</div><p class="sp-small sp-muted">Recent replies stay in this browser. Ideas can miss the mark; tell Claude what does not fit.</p>`;
  }
  async function openAI(prefill=''){
    if(prefill)aiDraft=prefill;screen='ai';notice='';render();
    if(await ensureSample()){aiStatus='ready';if(screen==='ai')render(false);return;}
    if(location.protocol==='file:'){aiStatus='unavailable';if(screen==='ai')render(false);return;}
  ...
  async function sendAI(){
    ...
    try{
      ...
      if(await ensureSample()){
        const convo=messages.map(m=>(m.role==='user'?'Person: ':'You: ')+m.content).join('\n\n');
        const out=await sampleFn(SPARK_BRIEF+'\n\n'+convo+'\n\nYou:',{signal:aiAbort.signal});
        result={reply:(out&&out.text)||'',truncated:!!(out&&out.truncated)};
      }else{
        ...
        try{result=await r.json();}catch{throw Error('No Claude connection in this copy. Your message has been kept.');}
