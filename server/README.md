...
| `IGDB_CLIENT_ID` + `IGDB_CLIENT_SECRET` *(or `TWITCH_CLIENT_ID` + `TWITCH_CLIENT_SECRET`)* | For IGDB | <https://dev.twitch.tv/console/apps> → Register Your Application (any name; OAuth Redirect URL `http://localhost`; category "Application Integration"). Then **Manage** → copy the Client ID → **New Secret** → copy the secret. IGDB is free and is the better of the two: it returns game modes, themes, perspectives and a real summary. The server exchanges these for an app token itself and refreshes it. |
...
| `APP_TOKEN` | Strongly recommended | Any string you invent. With it set, the **whole site is private**: every page shows a sign-in first, the browser keeps a year-long cookie (a hash of the token, never the token itself), and the API refuses anything without it. `/api/health` still answers Render's health check but says nothing about the setup. Without it, anyone who finds the URL can use the apps and spend your Anthropic credit. |
