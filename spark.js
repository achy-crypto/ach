...
(() => {
  ...
  style.textContent=`
  #spark-dialog{--sp-ink:#202a2b;--sp-muted:#52605d;--sp-line:#b5bdb7;--sp-paper:#f6f7f2;--sp-green:#146b61;box-sizing:border-box;color:var(--sp-ink);background:#e6e7e2;border:1px solid var(--sp-line);border-radius:12px;width:min(740px,calc(100% - 24px));max-height:calc(100dvh - 24px);padding:0;font:16px/1.55 system-ui,sans-serif;overscroll-behavior:contain}
  #spark-dialog::backdrop{background:rgba(15,26,25,.65)}
  #spark-dialog *{box-sizing:border-box}#spark-dialog [hidden]{display:none!important}
  #spark-dialog .sp-head{display:flex;align-items:center;justify-content:space-between;gap:16px;border-bottom:1px solid var(--sp-line);padding:16px 24px;background:#e6e7e2}
  #spark-dialog .sp-brand{font-size:14px;font-weight:700;letter-spacing:.04em}#spark-dialog .sp-brand::before{content:' ';display:inline-block;width:9px;height:9px;background:#c66a22;border-radius:50%;margin-right:8px}
  #spark-dialog .sp-body{padding:24px}#spark-dialog h1{font-size:clamp(26px,5vw,34px);line-height:1.2;margin:0 0 12px;letter-spacing:-.035em}#spark-dialog h2{font-size:21px;line-height:1.3;margin:0 0 8px}#spark-dialog h3{font-size:18px;margin:0 0 8px}
  #spark-dialog p{margin:8px 0 16px}#spark-dialog .sp-muted{color:var(--sp-muted)}#spark-dialog .sp-small{font-size:14px}
  #spark-dialog button,#spark-launch{font:inherit;line-height:1.35;cursor:pointer;touch-action:manipulation}#spark-dialog button{border:1px solid var(--sp-line);border-radius:6px;min-height:44px;background:var(--sp-paper);color:var(--sp-ink);padding:10px 14px;margin:0;box-shadow:none;text-transform:none;letter-spacing:normal}
  #spark-dialog button:hover{border-color:var(--sp-green);background:#fff}#spark-dialog button:focus-visible,#spark-dialog input:focus-visible,#spark-dialog textarea:focus-visible,#spark-dialog summary:focus-visible,#spark-launch:focus-visible{outline:3px solid #bd611b;outline-offset:3px}
  #spark-dialog button[aria-pressed=true],#spark-dialog .sp-primary{background:var(--sp-green);color:#fff;border-color:var(--sp-green)}#spark-dialog .sp-primary:hover{background:#0b5048}#spark-dialog .sp-close{min-width:44px}
  #spark-dialog .sp-row{display:flex;gap:8px;flex-wrap:wrap;align-items:center;margin:12px 0 20px}#spark-dialog .sp-card{border:1px solid var(--sp-line);border-left:3px solid var(--sp-green);background:var(--sp-paper);padding:20px;margin:12px 0;border-radius:3px}
  #spark-dialog .sp-card p{color:var(--sp-muted)}#spark-dialog .sp-label{display:block;font-size:14px;font-weight:700;margin:20px 0 8px}#spark-dialog .sp-kicker{font-size:13px;color:var(--sp-muted);letter-spacing:.07em;text-transform:uppercase;margin:0 0 8px}
  #spark-dialog input[type=text],#spark-dialog textarea{width:100%;border:1px solid #8a9690;border-radius:5px;background:#fff;color:var(--sp-ink);padding:12px;font:inherit;line-height:1.5}#spark-dialog textarea{min-height:96px;resize:vertical}
  ...
  #spark-dialog .sp-time{font-variant-numeric:tabular-nums;font-size:56px;font-weight:700;line-height:1.2;letter-spacing:-.04em;margin:20px 0 8px}#spark-dialog .sp-notice{border:1px solid #c08d55;background:#fff4e3;padding:16px;border-radius:6px;margin:16px 0}
  ...
  #spark-launch{position:fixed;bottom:18px;right:18px;z-index:9998;background:#146b61;color:white;border:1px solid #fff;border-radius:30px;padding:14px 20px;box-shadow:0 3px 16px #102b2433;font-family:system-ui,sans-serif;font-size:16px}
  #spark-home{width:100%;text-align:left;background:#f6f7f2;border:1px solid #adb9b1;border-left:3px solid #146b61;padding:18px;margin:0 0 20px;font:inherit;cursor:pointer;color:#202a2b}#spark-home strong,#spark-home span{display:block}#spark-home span{font-size:14px;margin-top:6px;color:#52605d}
  ...
  function ownView(){
    ...
    <label class="sp-label" for="spark-own-reason">What makes it unappealing?</label><select id="spark-own-reason" style="width:100%;padding:12px;font:inherit;background:white;color:#202a2b;border:1px solid #8a9690;border-radius:5px">${Object.entries({boring:'It sounds boring',big:'It feels too big',unclear:'I do not know where to begin',flat:'I am not feeling it'}).map(([k,v])=>`<option value="${k}" ${ownReason===k?'selected':''}>${v}</option>`).join('')}</select>
